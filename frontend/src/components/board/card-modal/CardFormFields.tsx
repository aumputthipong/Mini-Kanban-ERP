                ...
                {QUICK_DATE_OPTIONS.map((choice) => {
                  ...
                  return (
                    <button
                      key={choice.label}
                      type="button"
                      onClick={() => onChange("due_date")({ target: { value: formattedDate } } as React.ChangeEvent<HTMLInputElement>)}
                ...
                {QUICK_HOURS_OPTIONS.map((choice) => {
                  ...
                  return (
                    <button
                      key={choice.label}
                      type="button"
                      onClick={() => onChange("estimated_hours")({ target: { value: choice.value } } as React.ChangeEvent<HTMLInputElement>)}
                      className={`px-2.5 py-1 text-[11px] font-semibold rounded-md transition-colors border ${
                        isActive ? "bg-blue-50 border-blue-200 text-blue-700" : "bg-white border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-800"
                      }`}
                    >
